...
int main() {
    const char *cqi_file = "../data/1/cqi.csv";
    const char *bsr_file = "../data/1/bsr.csv";

    TBS_Table(); 

    for (int tti = 1; tti <= 15; tti++) {
        printf("\n=== TTI %d ===\n", tti);
        ...
        printf("[Scheduling] RoundRobin result at TTI %d:\n", tti);
        for (int i = 0; i < MAX_UE; i++) {
...
void MaxC_I(UEData *ue_data, SchedulerResponse *response) {
    int scheduled = 0;
    int rb_per_ue = NUM_RB / MAX_UE_PER_TTI;
