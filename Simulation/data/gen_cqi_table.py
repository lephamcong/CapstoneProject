...
def scenario_high_traffic():
    return random_cqi(6, 9), random_bsr(8000, 10000)

# 2. low_traffic: CQI cao, BSR thấp
def scenario_low_traffic():
    return random_cqi(10, 15), random_bsr(1000, 2000)
...
def scenario_burst_traffic():
    ...
    for ue in range(NUM_UE):
        ...
        for tti in range(NUM_TTI):
            if tti % 500 < 100:
                row.append(random.randint(8000, 10000))  # burst
            else:
                row.append(random.randint(1000, 3000))
        bsr.append(row)
    return cqi, bsr
...
def scenario_ideal_condition():
    ...
    bsr = random_bsr(4000, 7000)  # traffic vừa phải
    return cqi, bsr
...
def scenario_ideal_condition_bsr10000():
    ...
    bsr = random_bsr(10000,10000)  # traffic vừa phải
    return cqi, bsr
...
# write_dual_scenario("mixed", *scenario_mixed())
